...
const HeadTodayFollow = () => {
  ...
  const handleSearch = () => {
    const fData = htotalTodayFUp?.filter(
      (si) =>
  ...
  const handleInputChange = (event) => {
    ...
    // console.log(value);
    const fiData = htotalTodayFUp.filter(
  ...
  return (
    <div className="mx-2 my-6">
      <h3 className="text-2xl mb-3">
        {user.name}'s Today Followup Students : {htotalTodayFUp.length}
      </h3>
...
export default HeadTodayFollow;
