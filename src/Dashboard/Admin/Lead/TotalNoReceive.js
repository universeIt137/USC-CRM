...
const TotalNoReceive = () => {
  const { user } = useContext(AuthContext);
  const [totalNoReceive, setTotalNoReceive] = useState([]);
  ...
  const uniqueCourse = [
    ...new Set(totalNoReceive?.map((user) => user?.course?.name)),
  ];
  ...
  function handleCourseChange(event) {
    ...
    const fData = totalNoReceive?.filter(
  ...
  const handleInputChange = (event) => {
    ...
    // console.log(value);
    const fiData = totalNoReceive.filter(
        ...
        <ol className="flex h-8 space-x-2">
          ...
          <li className="flex items-center space-x-2">
            ...
            <a
              ...
              className="flex items-center px-1 capitalize hover:underline"
            >
              Total No Receive Students
            </a>
          </li>
        </ol>
      </nav>
      {/* <h3 className="text-3xl">Total No Receive Students : {totalNoReceive.length}</h3> */}
              ...
              <tbody className="w-fit text-xs">
                {filterData.length > 0 &&
                  filterData
                    ...
                    ?.map((noreceive, i) => (
                      <tr key={noreceive._id}>
                        <th className="p-1 border-2">{i + 1}</th>
                        <td className="p-1 border-2">
                          {noreceive.date.slice(0, 10)}
                        </td>
                        <td className="p-1 border-2">
                          {noreceive.course.name}
                        </td>
                        ...
                        <td className="p-1 border-2">
                          <p
                            ...
                            onClick={() => handleDelete(noreceive._id)}
                          >
                            Delete
                          </p>
                        </td>
