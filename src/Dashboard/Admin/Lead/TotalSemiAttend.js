...
const TotalSemiAttend = () => {
  const { user } = useContext(AuthContext);
  const [totalSemiAttend, setTotalSemiAttend] = useState([]);
  ...
  useEffect(() => {
    fetch("https://demo-usc-crm-software.vercel.app/leads?seminarAttend=true")
      .then((response) => response.json())
      .then((data) => {
        setFilterData(data);
        setTotalSemiAttend(data);
      });
  }, []);
  ...
  const uniqueCourse = [
    ...new Set(totalSemiAttend?.map((user) => user?.course?.name)),
  ];
  ...
  function handleCourseChange(event) {
    ...
    const fData = totalSemiAttend?.filter(
  ...
  const handleInputChange = (event) => {
    ...
    // console.log(value);
    const fiData = totalSemiAttend.filter(
  ...
  const handleDelete = (semiattend) => {
    ...
    fetch(`https://demo-usc-crm-software.vercel.app/delete/${semiattend}`, {
      method: "DELETE",
      headers: {
        ...
        <ol className="flex h-8 space-x-2">
          ...
          <li className="flex items-center space-x-2">
            ...
            <a
              ...
              className="flex items-center px-1 capitalize hover:underline"
            >
              Total Seminar Attends Students
            </a>
          </li>
        </ol>
      </nav>
      {/* <h3 className="text-3xl">Total Seminar Attends Students : {totalSemiAttend.length}</h3> */}
              ...
              <tbody className="w-fit text-xs">
                {filterData.length > 0 &&
                  filterData
                    ...
                    ?.map((semiattend, i) => (
                      <tr key={semiattend._id}>
                        <th className="p-1 border-2">{i + 1}</th>
                        <td className="p-1 border-2">
                          {semiattend.date.slice(0, 10)}
                        </td>
                        <td className="p-1 border-2">
                          {semiattend.course.name}
                        </td>
                        <td className="p-1 border-2">
                          {semiattend.batch.name}
                        </td>
                        <td className="p-1 border-2">{semiattend.user.name}</td>
                        <td className="p-1 border-2">{semiattend.head.name}</td>
                        <td className="p-1 border-2">{semiattend.name}</td>
                        <td className="p-1 border-2">{semiattend.phone}</td>
                        <td className="p-1 border-2">{semiattend.email}</td>
                        <td className="p-1 border-2">
                          <p
                            className="btn btn-xs btn-denger"
                            onClick={() => handleDelete(semiattend._id)}
                          >
                            Delete
                          </p>
                        </td>
...
export default TotalSemiAttend;
