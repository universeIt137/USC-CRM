...
const TotalLead = () => {
  ...
  useEffect(() => {
    fetch("https://demo-usc-crm-software.vercel.app/leads")
  ...
  const uniqueCourse = [
    ...new Set(totalleads?.map((user) => user?.course?.name)),
  ];
  ...
  const handleInputChange = (event) => {
    ...
    // console.log(value);
    const fiData = totalleads.filter(
                  ...
                  filterData
                    ...
                    ?.map((leads, i) => (
                      <tr key={leads._id}>
                        ...
                        <td className="p-1 border-2">
                          {leads?.email ? leads?.email : "Email is not submit"}
                        </td>
                        <td className="p-1 border-2">
                          <p
